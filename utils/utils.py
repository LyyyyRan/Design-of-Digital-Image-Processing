...
def show_results(img, boxes):
    ...
    for box in boxes:
        ...
        if score > 0.65:
            ...
            x2 = x1 + box[2]
            y2 = y1 + box[3]
            cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), thickness=tl)


def drawLandmark_multiple(img, bbox, landmark, flag=None):
    h, w, c = img.shape
    tl = round(0.002 * (h + w) / 2) + 1  # line/font thickness
    '''
    Input:
    ...
    # color:
    face_bbox_color = (100, 0, 240)
    mouth_point_color = (100, 100, 200)
    text_color = (0, 100, 0)

    cv2.rectangle(img, (bbox.left, bbox.top), (bbox.right, bbox.bottom), face_bbox_color, tl)

    for x, y in landmark:
        cv2.circle(img, (int(x), int(y)), tl, mouth_point_color, -1)

    if flag:
        if flag[0]:
            text_color = (0, 180, 200)
            text = "Open Enough:{:.4f}".format(flag[1])
        else:
            text = "Not Enough:{:.4f}".format(flag[1])
        cx = bbox.left
        cy = bbox.bottom + 25
        cv2.putText(img, text, (cx, cy), cv2.FONT_HERSHEY_DUPLEX, 1.5, text_color, tl)
...
def drawLandmark_Attribute(img, bbox, landmark, gender, age):
    ...
    for x, y in landmark:
        ...
        if age.argmax() == 0:
            ...
            cv2.putText(img, 'young', (int(bbox.right), int(bbox.bottom)), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0),
                        3)
    return img
